...
**언제 `/handoff` 를 치나?** — 세션을 접기 직전. 커밋 직전이나 자리를 뜨기 전이 딱 좋다.
자잘한 세션마다 칠 필요는 없고, "남한테 인수인계할 게 생긴" 세션이면 친다.
...
안 건드릴 거면 무시해도 된다. Claude에게 "그 엔트리 읽고 요약해줘" 하면 알아서 본다.

**`verified` / `unverified` 는 누가 정하나?** — Claude가 이번 세션 기준으로 판단한다.
테스트를 실제로 돌려 통과했거나 커밋으로 확인된 것만 `verified`. 애매하면 `unverified`.
"했다"와 "됐다"를 일부러 구분하는 게 핵심이라, 낙관적으로 올리지 않는다.

**시크릿(키·토큰)이 기록에 새지 않나?** — `/handoff`가 쓰기 전에 스스로 검사하고,
본문에는 코드·diff를 안 붙이고 커밋 해시·경로로만 참조한다. 그래도 push 전 훑어보는 걸 권장.

**토큰(비용) 안 늘어나나?** — 세션당 자동 주입은 **2KB 상한**으로 고정. 나머지 상세 기록은
필요할 때만 grep/read로 꺼내 본다. 저널이 아무리 쌓여도 매 세션 비용은 그대로다.

**팀원이 안 쓰면 나만 손해 아닌가?** — 아니다. 나 혼자 써도 "지난주 내가 뭘 미검증으로
남겼는지"를 이번 주 내 Claude가 알려준다. 팀원이 켜지면 그 효과가 곱해질 뿐.
